...
class EmbeddingGenerator:
    """
    Generates embeddings for text chunks using Gemini embedding model.
    Implements:
    - single embedding
    - batch embeddings
    - caching
    ...
    def __init__(self, model_name="text-embedding-004", cache_path="data/embedding_cache.json"):
        load_dotenv()
    ...
    def generate_embedding(self, text: str):
        ...
        # Basic validation (Gemini embeddings may vary in size)
        if not embedding or len(embedding) < 100:
            raise RuntimeError("Invalid embedding received")
    ...
    def prepare_embedding_data(self, embedded_chunks):
        ...
        for item in embedded_chunks:
            ...
            # Basic validation
            if not embedding:
                raise RuntimeError("Invalid embedding during prepare step")
